...
// clang-format off
struct SendStruct {
  float yaw;                   // 目标位置欧拉角yaw值
  float pitch;                 // 目标位置欧拉角pitch值
  int    shoot;                 // 开火状态 0->不开火 1->1的范围内一直开火 2->跳变到2开火
  float time_stamp;            // 当前视觉程序时间戳,用于时间同步,默认-1不进行同步 单位s
  int    required_image_width;  // 需要获得的视觉画面的宽度,默认-1不进行修改
  int    required_image_height; // 需要获得的视觉画面的高度,默认-1不进行修改
  SendStruct() = default;
  SendStruct(float yaw, float pitch, int shoot, float time_stamp = -1,
